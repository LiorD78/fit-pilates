# Lokální citace — copy-paste balíček pro registrace

> **Cíl:** NAP (Name, Address, Phone) konzistence napříč adresáři. Lokální citace = druhý nejsilnější ranking signál pro lokální Google SEO po GBP.
> **Pravidlo:** **Identický NAP** všude. Každá odchylka snižuje trust.
> **Updated:** 2026-04-28 (přidána silovka + box)

---

## ⭐ PRIORITA — registrovat v tomto pořadí

| # | Site | Týden | Důležitost | Status |
|---|------|-------|------------|--------|
| 1 | Google Business Profile | 1 | ★★★★★ kritická | ⏳ |
| 2 | **Firmy.cz** (Seznam) | 1 | ★★★★★ vysoká | ⏳ |
| 3 | **Apple Business Connect** | 2 | ★★★★ rostoucí | ⏳ |
| 4 | **Najisto.cz** | 2 | ★★★★ | ⏳ |
| 5 | **Bing Places** | 3 | ★★★ | ⏳ |
| 6 | Zlatestranky.cz | 3 | ★★★ | ⏳ |
| 7 | Sluzby.cz | 4 | ★★ | ⏳ |
| 8 | iZurnal.cz | 4 | ★★ | ⏳ |
| 9 | SportCentral.cz | 5 | ★★ | ⏳ |
| 10 | Hyperinzerce.cz | 5 | ★ | ⏳ |

---

## 📋 MASTER NAP — copy-paste přesně toto

```
Název firmy:    Fit Pilates
Slogan:         Pilates · Fitness · Box
Adresa:         U Trati 886/52
Město:          Praha 10
Část:           Strašnice
PSČ:            100 00
Stát:           Česká republika
Telefon:        +420 604 925 249
WhatsApp:       +420 604 925 249
E-mail:         stefan@fitpilates.cz
Web:            https://fitpilates.cz
Instagram:      https://www.instagram.com/stefanbitto
Facebook:       https://www.facebook.com/share/1Hs5UmhdH9/

Otevírací doba: Po-Pá 7:00-20:00
                So-Ne dle dohody (kontaktujte telefonicky/WhatsApp)

GPS:            50.0653696, 14.4951578

Kategorie:      Pilates studio (primární)
                Fitness centrum (sekundární)
                Osobní trenér / Personal training
                Sportovní klub / Studio
                Boxerský trénink

IČO:            ____________  (vyplň před registrací)
DIČ:            ____________  (vyplň před registrací)
```

---

## 📝 POPISY — různé délky podle limitu adresáře

### Verze 1: 150 znaků (Twitter/krátký box)

```
Pilates studio v Praze 10. Reformer pilates, silový trénink a kondiční box pod jednou střechou. Vede Štefan Bitto, ACI trenér od 2000.
```

### Verze 2: 250 znaků (Firmy.cz krátký, Najisto)

```
Pilates studio v Praze 10 - Strašnice. Reformer pilates na Allegro reformerech (jeden s Tower) plus Cadillac, silový trénink a kondiční box pod jednou střechou. Vede Štefan Bitto, ACI trenér fitness I. třídy od roku 2000. Vstupní lekce 590 Kč.
```

### Verze 3: 500 znaků (běžný adresář)

```
Fit Pilates je studio v Praze 10 – Strašnice, kde najdete pilates, silový trénink a kondiční box pod jednou střechou. Cvičíme na profesionálních Balanced Body Allegro reformerech (jeden s přídavnou věží Tower) a Cadillacu (Trapeze Table). Silová zóna obsahuje TuffStuff squat rack, Jordan činky, kettlebelly Escape, TRX a BOSU Elite. Box vybavený rukavicemi Windy a King, lapy Twins, pytel Everlast – vše bez sparringu. Vede Štefan Bitto, ACI trenér od 2000, 24+ let praxe. Vstupní lekce 590 Kč.
```

### Verze 4: 1000 znaků (rozšířený adresář, GBP)

```
Fit Pilates je studio v Praze 10 – Strašnice s ojedinělou kombinací tří disciplín pod jednou střechou: reformer pilates, silový trénink a kondiční box. Žádné jiné studio v Praze 10 tuto kombinaci s jedním lektorem nenabízí.

Vybavení:
• 3× Balanced Body Allegro reformer (jeden s přídavnou věží Tower)
• Cadillac (Trapeze Table) Balanced Body s chrómovaným Speed-Rail rámem
• Kompletní silová zóna: TuffStuff squat rack, Jordan činky, kettlebelly Escape, TRX závěsný systém, BOSU Elite
• Profesionální boxerské vybavení: rukavice Windy a King, lapy Twins, pytel Everlast (bez sparringu)

Vede Štefan Bitto, ACI trenér fitness I. třídy od roku 2000. Osobní trenér od 2001 – 24+ let praxe. Certifikovaný instruktor pilates. Specializace: silový a funkční trénink, kondiční box, pilates reformer.

Studio: U Trati 886/52, Praha 10 – Strašnice. MHD: Tram Radošovická, Bus Korytná. Parkování zdarma v ulici. Po–Pá 7:00–20:00, So–Ne dle dohody.

Vstupní zkušební lekce 590 Kč (60 minut). Telefon a WhatsApp: +420 604 925 249.
```

### Verze 5: 2000 znaků (dlouhá verze pro adresáře bez limitu)

```
Fit Pilates je studio v Praze 10 – Strašnice. Hlavní výhodou je kombinace tří disciplín pod jednou střechou s jedním lektorem: reformer pilates, silový a funkční trénink, kondiční box. Tato kombinace je v Praze 10 ojedinělá – většina studií dělá jen jednu věc, my všechny tři.

PILATES
Cvičíme na třech profesionálních Balanced Body Allegro reformerech, z nichž jeden má přídavnou věž (Tower) pro pokročilé cvičení. Studio má také plný Cadillac (Trapeze Table) Balanced Body s chrómovaným Speed-Rail rámem – nejvšestrannější pilates stroj. Pilates je vhodný pro začátečníky, pokročilé, sportovce, lidi v rehabilitaci i postnatální klientky.

SILOVÝ A FUNKČNÍ TRÉNINK
Kompletní silová zóna: TuffStuff squat rack (USA), kompletní sada Jordan činek (UK profi standard), olympijská osa s kotouči, nastavitelná lavička, kettlebelly Escape, TRX závěsný systém, BOSU Elite. Trénink cílíme podle klienta – od základů techniky až po pokročilé compound exercises. Vhodné pro začátečníky, pokročilé, sportovce a klienty v rehabilitaci.

KONDIČNÍ BOX
Profesionální vybavení: rukavice Windy a King (thajské brandy s 50letou tradicí), lapy Twins, boxerský pytel Everlast. Bez sparringu – žádný kontakt mezi cvičenci. Cvičíme techniku úderů, stínový box, práci na pytli a lapování s trenérem. Ideální kardio a stres relief pro lidi z kanceláře.

LEKTOR
Štefan Bitto: ACI trenér fitness I. třídy od roku 2000. Osobní trenér od 2001 – 24+ let praxe. Certifikovaný instruktor pilates. Vede všechny lekce osobně.

PRAKTICKÉ INFORMACE
Adresa: U Trati 886/52, 100 00 Praha 10 – Strašnice
Telefon a WhatsApp: +420 604 925 249
E-mail: stefan@fitpilates.cz
Web: https://fitpilates.cz
MHD: Tram Radošovická + Bus Korytná
Parkování: zdarma v ulici
Otevřeno: Po–Pá 7:00–20:00, So–Ne dle dohody
Vstupní lekce: 590 Kč (60 minut, individuální)
```

---

## 🏷️ KEYWORDS — pro tag/keyword pole adresářů

```
pilates Praha
pilates Praha 10
reformer pilates
pilates Strašnice
Allegro reformer
silový trénink Praha
silový trénink Praha 10
kondiční box Praha
fitness boxing
funkční trénink Praha
osobní trenér Praha 10
fitness Praha 10
Štefan Bitto
Fit Pilates
pilates pro začátečníky
pilates rehabilitace
postnatální pilates
TRX trénink
kettlebell Praha
```

---

## 🖼️ FOTKY — co nahrát

| Pořadí | Soubor | Použití |
|---|---|---|
| 1 (hlavní) | `images/hero.jpg` (1728×1129) | Cover photo |
| 2 | `images/logo-dark.png` | Logo (square) |
| 3 | `images/studio-main.jpg` | Studio interiér |
| 4 | `images/pilates.jpg` | Reformer pilates |
| 5 | `images/fitness.jpg` | Silová zóna |
| 6 | `images/boxing-studio.jpg` | Boxerské vybavení |
| 7 | `images/cadillac-detail.jpg` | Cadillac (premium signal) |
| 8 | `images/trainer-hero.jpg` | Štefan portrét |
| 9 | `images/trainer-fitness.jpg` | Štefan v akci |

**Tip:** vždy nahrát logo + minimum 5 fotek. Adresáře s víc fotkami rankujou výš.

---

## 📍 SPECIFICKÉ NÁVODY pro top 5 sites

### 1. Firmy.cz (Seznam) — KRITICKÉ pro CZ
- URL registrace: https://www.firmy.cz/zaradit-firmu
- ⚠️ **POZOR:** Pokud existuje starší profil „Osobní trenér Štefan Bitto, Praha-Háje", **NEPŘEPISUJ ho** — vytvoř nový pro studio Fit Pilates jako samostatnou entitu (nebo edituj starý a změň na novou adresu).
- Kategorie: **Sport a relaxace → Pilates studio**, secondary: **Sport a relaxace → Fitness centrum**
- Popis: použij **Verzi 4 (1000 znaků)**
- Fotky: nahrát všech 9 (Firmy.cz povoluje až 20 fotek)
- Otevírací doba: vyplnit Po-Pá 7-20, So-Ne nechat **„Otevřeno na zavolání"** (Firmy.cz to umí)

### 2. Apple Business Connect (Apple Maps)
- URL: https://businessconnect.apple.com
- Vyžaduje: Apple ID + ověření telefonem
- Kategorie: **Health & Fitness → Pilates Studio** (nebo Fitness Center)
- Popis: použij **Verzi 4 (1000 znaků)**
- ⭐ **Tip:** Apple Maps se na iPhone používá víc než Google na iOS. Nezanedbávat.

### 3. Najisto.cz
- URL: https://www.najisto.cz/registrace
- Limit popisu: ~500 znaků → použij **Verzi 3**
- Kategorie: **Sport → Fitness, Pilates**
- Bonus: lze přidat 5 fotek + odkazy na sociální sítě

### 4. Bing Places (= Microsoft Bing + Yahoo + DuckDuckGo)
- URL: https://www.bingplaces.com/
- Vyžaduje: Microsoft účet
- Pokud máš GBP, **lze importovat** rovnou (úspora času)
- Po importu: zkontroluj NAP, doplň fotky, popis
- Důležitost: **roste** s ChatGPT/Bing Chat indexací

### 5. Zlatestranky.cz
- URL: https://www.zlatestranky.cz/zarazeni-firmy
- Tradiční telefonní seznam, **starší klientela**
- Limit popisu: ~250 znaků → použij **Verzi 2**

---

## ✅ POSTUP REGISTRACE — checklist pro každý web

Pro KAŽDOU registraci postupuj stejně:

1. ☐ Použij **přesně stejný název** „Fit Pilates" (NE „Fit Pilates Studio Praha 10", NE „FitPilates")
2. ☐ Adresa **ZNAK PO ZNAKU stejná**: „U Trati 886/52, 100 00 Praha 10 - Strašnice"
3. ☐ Telefon ve formátu **+420 604 925 249** (s mezerami!)
4. ☐ E-mail **stefan@fitpilates.cz**
5. ☐ Web **https://fitpilates.cz** (s https, BEZ www)
6. ☐ Kategorie: hlavní **Pilates studio**, sekundární Fitness/Sport
7. ☐ Popis vyber podle limitu (Verze 1-5 výše)
8. ☐ Nahraj **logo + 5+ fotek** ze seznamu
9. ☐ Otevírací doba Po-Pá 7-20, So-Ne podle možnosti adresáře
10. ☐ Zaznamenej do tabulky níže: **datum + URL profilu**

---

## 📊 Tabulka registrací (vyplňuj postupně)

| # | Site | Datum | URL profilu | Status |
|---|------|-------|-------------|--------|
| 1 | Google Business Profile | | | ⏳ |
| 2 | Firmy.cz | | | ⏳ |
| 3 | Apple Business Connect | | | ⏳ |
| 4 | Najisto.cz | | | ⏳ |
| 5 | Bing Places | | | ⏳ |
| 6 | Zlatestranky.cz | | | ⏳ |
| 7 | Sluzby.cz | | | ⏳ |
| 8 | iZurnal.cz | | | ⏳ |
| 9 | SportCentral.cz | | | ⏳ |
| 10 | Hyperinzerce.cz | | | ⏳ |

---

## 🔍 NAP konzistence audit (po dokončení všech registrací)

Stejný NAP musí být na **každém** níže uvedeném místě. Audituj 1× za 3 měsíce.

### Web fitpilates.cz
- ✅ Schema.org HealthClub (homepage)
- ✅ Footer všech stránek
- ✅ Kontaktní sekce (homepage)
- ✅ /o-stefanovi/ Person schema
- ✅ /pilates-praha-10/ adresa + GPS

### GBP profil ✅
### Všechny lokální adresáře (10× výše) ☐

### Sociální sítě
- ✅ Instagram bio (@stefanbitto)
- ✅ Facebook About
- ☐ LinkedIn (nepovinné, ale vhodné pro B2B)

---

## ⚠️ ČASTÉ CHYBY — vyhni se jim

| ❌ Špatně | ✅ Správně |
|---|---|
| `+420-604-925-249` | `+420 604 925 249` |
| `Praha 10` (bez Strašnice) | `Praha 10 - Strašnice` |
| `https://www.fitpilates.cz` | `https://fitpilates.cz` (BEZ www) |
| `info@fitpilates.cz` | `stefan@fitpilates.cz` |
| `Fit Pilates Studio Praha 10` | `Fit Pilates` (krátké, čisté) |
| `100 00 Praha 10` (bez ulice) | `U Trati 886/52, 100 00 Praha 10` |

---

## 📈 Měření efektu (Search Console + GA4)

Po **60 dnech** od první registrace porovnej:

| Metrika | Před (28.4.2026) | Po (28.6.2026) | Δ |
|---|---|---|---|
| Impressions na "pilates Praha 10" | | | |
| Impressions na "pilates Strašnice" | | | |
| Impressions na "silový trénink Praha 10" | | | |
| Impressions na "kondiční box Praha 10" | | | |
| Impressions na brand "Fit Pilates" | | | |
| Direct traffic (z citací) | | | |
| Backlinky (Ahrefs/SEMrush check) | | | |
| GBP profil zobrazení | | | |
| GBP kliknutí na "navigovat" | | | |

---

## 🎯 Cíle (90 dnů od registrace)

- **Top 10** na Google CZ pro „pilates Praha 10"
- **Top 5** pro „pilates Strašnice"
- **Top 3** pro brand „Fit Pilates"
- **+50%** brand impressions v Search Console
- **5+** GBP recenzí (5★ průměr)

---

*Připraveno: 26.4.2026 | Aktualizováno: 28.4.2026 (přidána silovka + box) | Související: GBP-CHECKLIST.md, GBP-POSTS-RECENZE.md, /pilates-praha-10/*
